...
HANAVER=${HANAVER^^}
if [ "${HANAVER}" = "SAP HANA PLATFORM EDITION 2.0 SPS01 REV 10 (51052030)" ]; then
  hanapackage="51052030"
else
  echo "not 51052030"
  if [ "$HANAVER" = "SAP HANA PLATFORM EDITION 2.0 SPS02 (51052325)" ]; then
    hanapackage="51052325"
  else
    echo "not 51052325"
    if [ "$HANAVER" = "SAP HANA PLATFORM EDITION 2.0 SPS03 REV30 (51053061)" ]; then
      hanapackage="51053061"
    else
      if [ "$HANAVER" = "SAP HANA PLATFORM EDITION 2.0 SPS04 REV40 (51053787)" ]; then
...
rc=$?
if [[ $rc != 0 ]]; then
  retry 5 "/usr/bin/wget --quiet https://raw.githubusercontent.com/bqtrinh/AzQ/masterhdbinst.cfg"
fi
